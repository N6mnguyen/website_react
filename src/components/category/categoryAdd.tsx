...
const AddProduct = ({ add }: Props) => {
    const { register, handleSubmit,formState:{errors} } = useForm<FormCate>();
    ...
    return (
        ...
        <div className="">
            ...
            <form className='flex flex-col py-12 w-[500px] mx-auto gap-4' onSubmit={handleSubmit(onSubmit)}>
                ...
                <input className='py-2 px-4 border border-2 rounded-[20px]' type='text' {...register('itemCount',{pattern:/^\d*$/})} placeholder='Số lượng'required />
                {(errors.itemCount) && <span>số lượng là số không âm</span>}
